...
@section('content')
    @php
        $taksi = App\Models\Pemesanan::where('id_user', Auth::user()->id)
            ->where('pesanan_selesai', 0)
            ->latest()
            ->first();
        $detail_taksi = App\Models\Taksi::find($taksi->id_taksi);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(item) {
                            ...
                            var markerTujuan = L.marker([item.tujuan.latitude, item.tujuan
                                    .longitude
                                ], {
                                    draggable: false
                                }).addTo(map)
                                .bindPopup(item.tujuan.nama_lokasi);
                        });
                        ...
                        var waypoints2 = data.map(function(item) {
                            return L.latLng(item.tujuan.latitude, item.tujuan.longitude);
                        });
